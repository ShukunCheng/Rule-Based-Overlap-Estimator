...
if __name__ == '__main__':
    ...
    beta_array = [round(x, 2) for x in np.arange(0.01, 1, 0.02)]
    res = {}
    index = 1
    ...
    for i in range(index):
        ...
        res[dim] = {"br_mean": np.zeros(len(beta_array)),
                    "br_var": np.zeros(len(beta_array)),
                    "svm_mean": np.zeros(len(beta_array)),
                    "svm_var": np.zeros(len(beta_array))}
    ...
    for i in range(index):
        ...
        plt.figure()
        plt.plot(beta_array, br_mean, label="Overrule", marker=markers[0])
        plt.plot(beta_array, svm_mean, label="One Class SVM", marker=markers[1])
        plt.fill_between(beta_array, [br_mean[i] - br_var[i] for i in range(len(br_mean))],
                         [br_mean[i] + br_var[i] for i in range(len(br_mean))], alpha=0.5)
        plt.fill_between(beta_array, [svm_mean[i] - svm_var[i] for i in range(len(svm_mean))],
